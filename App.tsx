...
type DmxProduct = { id: string; name: string; price: number; image?: string; note?: string; stock?: number; sold?: number; hidden?: boolean; codesPreview?: string[] };
type DmxOrder = { id: string; userId?: string; username?: string; productName: string; price: number; image?: string; code: string; note?: string; createdAt: string };
type Topup = { id: string; userId: string; username: string; amount: number; note?: string; status: string; createdAt: string };
type ProviderSettings = { codesimEnabled: boolean; codesimApiKeyMasked: string; hasCodesimApiKey: boolean };
...
export default function App() {
  ...
  const [providerSettings, setProviderSettings] = useState<ProviderSettings>({ codesimEnabled: true, codesimApiKeyMasked: "", hasCodesimApiKey: false });
  ...
  const loadSettings = async () => { const res = await fetch("/api/settings"); if (res.ok) setSettings({ ...defaultSettings, ...(await res.json()) }); };
  ...
  const loadDmxProducts = async () => { const res = await fetch("/api/dmx/products"); if (res.ok) setDmxProducts(await res.json()); };
  const loadDmxOrders = async () => { if (!user) return; const res = await fetch(`/api/dmx/orders?userId=${user.id}`); if (res.ok) setDmxOrders(await res.json()); };
  const loadAdminDmx = async () => { if (!user || !isAdmin) return; const res = await fetch("/api/admin/dmx/products", { headers }); if (res.ok) setDmxProducts(await res.json()); };

  useEffect(() => { loadSettings(); loadServices(); loadDmxProducts(); }, []);
  useEffect(() => { if (user) { loadOrders(); loadTopups(); loadUsers(); loadAdminServices(); loadProviderSettings();
      loadDmxOrders();
      loadAdminDmx(); } }, [user, tab]);
  useEffect(() => {
    if (!user) return;
  ...
  const saveProviderSettings = async () => {
    ...
    setCodesimApiKey(""); setProviderSettings(data); await loadAdminServices(); await loadServices(); show("Đã lưu cấu hình CodeSim");
  };
  const testProvider = async () => {
    const res = await fetch("/api/admin/provider-test", { headers });
    const data = await res.json(); setProviderTest(data); show(data.ok ? "CodeSim API hoạt động" : "CodeSim API lỗi");
  };
  ...
  const buyDmx = async (product: DmxProduct) => {
    ...
    const res = await fetch("/api/dmx/buy", { method: "POST", headers, body: JSON.stringify({ userId: user.id, productId: product.id }) });
    const data = await res.json();
  ...
  const createDmxProduct = async () => {
    const res = await fetch("/api/admin/dmx/products", { method: "POST", headers, body: JSON.stringify({ name: newDmxName, price: Number(newDmxPrice || 0), image: newDmxImage, note: newDmxNote }) });
    const data = await res.json();
    if (!res.ok) return show(data.message || "Lỗi tạo sản phẩm");
    setNewDmxName(""); setNewDmxPrice(""); setNewDmxImage(""); setNewDmxNote("");
    await loadAdminDmx();
    show("Đã tạo sản phẩm");
  };

  const updateDmxProduct = async (product: DmxProduct, patch: any) => {
    const res = await fetch(`/api/admin/dmx/products/${product.id}`, { method: "PUT", headers, body: JSON.stringify(patch) });
    const data = await res.json();
  ...
  const uploadDmxCodes = async (product: DmxProduct) => {
    ...
    if (!codes) return;
    const res = await fetch(`/api/admin/dmx/products/${product.id}/codes`, { method: "POST", headers, body: JSON.stringify({ codes }) });
    const data = await res.json();
  ...
  const clearDmxCodes = async (product: DmxProduct) => {
    if (!confirm(`Xóa toàn bộ mã tồn kho của ${product.name}?`)) return;
    const res = await fetch(`/api/admin/dmx/products/${product.id}/codes`, { method: "DELETE", headers });
    const data = await res.json();
    if (!res.ok) return show(data.message || "Lỗi xóa mã");
    await loadAdminDmx();
    show("Đã xóa toàn bộ mã tồn kho");
  };

  const deleteDmxProduct = async (product: DmxProduct) => {
    if (!confirm(`Xóa sản phẩm ${product.name}?`)) return;
    const res = await fetch(`/api/admin/dmx/products/${product.id}`, { method: "DELETE", headers });
    const data = await res.json();
  ...
  if (!user) return (
    <div className={`min-h-screen ${settings.background} flex items-center justify-center p-4`}>
      ...
      <form onSubmit={isLogin ? login : register} className="bg-white rounded-3xl p-8 shadow max-w-md w-full">
        <div className="w-16 h-16 bg-indigo-600 text-white rounded-2xl grid place-items-center font-black text-2xl mb-4 overflow-hidden">{settings.logoImage ? <img src={settings.logoImage} className="w-full h-full object-cover" /> : settings.logoText}</div>
  ...
  return (
    <div className="min-h-screen bg-slate-100">
      ...
      <aside className="fixed left-0 top-0 h-full w-72 bg-slate-950 text-white p-5 hidden lg:block overflow-y-auto">
        <div className="flex items-center gap-3 mb-8"><div className="w-12 h-12 bg-indigo-600 rounded-2xl grid place-items-center font-black overflow-hidden">{settings.logoImage ? <img src={settings.logoImage} className="w-full h-full object-cover" /> : settings.logoText}</div><h2 className="font-black text-xl">{settings.siteName}</h2></div>
      ...
      <main className="lg:ml-72 p-4 lg:p-8">
        <div className="lg:hidden flex gap-2 overflow-x-auto mb-4">
          {["services", "orders", "dmx", "topup", ...(isAdmin ? ["adminTopups", "users", "adminServices", "adminDmx", "api", "settings"] : [])].map(t => <button key={t} onClick={() => setTab(t)} className="bg-slate-900 text-white rounded-xl px-4 py-2 whitespace-nowrap">{t}</button>)}
        </div>
        ...
        {!isAdmin && activeOrders.length > 0 && <Panel title="Sim đang thuê / chờ OTP">
          ...
          <div className="space-y-3">{activeOrders.map(o => <OrderBox key={o.id} order={o} isAdmin={false} onCheck={checkCode} onReuse={reuseOrder} />)}</div>
        </Panel>}
        ...
        {tab === "orders" && <Panel title="Lịch sử thuê số"><div className="space-y-3">{orders.map(o => <OrderBox key={o.id} order={o} isAdmin={isAdmin} users={users} onCheck={checkCode} onReuse={reuseOrder} />)}</div></Panel>}

        {tab === "dmx" && <Panel title="Dịch vụ DMX">
          {!dmxProducts.length && <div className="bg-amber-50 border border-amber-200 rounded-2xl p-4 mb-4">Hiện chưa có voucher DMX nào đang bán.</div>}
          <div className="grid md:grid-cols-2 xl:grid-cols-3 gap-4">
            {dmxProducts.map(p => <div key={p.id} className="bg-white border rounded-3xl p-5 shadow-sm">
              ...
              <button disabled={busy || !p.stock} onClick={() => buyDmx(p)} className="mt-5 w-full bg-indigo-600 text-white rounded-2xl py-3 font-black">Mua ngay</button>
            </div>)}
          </div>
          <div className="mt-10">
            <h2 className="text-2xl font-black mb-4">Lịch sử mua voucher</h2>
            <div className="space-y-3">{dmxOrders.map(o => <div key={o.id} className="border rounded-2xl p-4 bg-white"><div className="flex gap-4">{o.image && <img src={o.image} className="w-28 h-28 object-cover rounded-2xl" />}<div className="flex-1"><h3 className="font-black text-xl">{o.productName}</h3><p className="text-sm text-slate-500">Giá: {money(o.price)} | {new Date(o.createdAt).toLocaleString("vi-VN")}</p><div className="mt-3 bg-emerald-50 border border-emerald-200 rounded-xl p-3"><b>{o.code}</b></div>{o.note && <div className="mt-3 bg-slate-100 rounded-xl p-3 text-sm whitespace-pre-wrap">{o.note}</div>}</div></div></div>)}</div>
          </div>
        </Panel>}
        ...
        {tab === "adminDmx" && isAdmin && <Panel title="Quản lý DMX">
          <div className="bg-white border rounded-3xl p-5 mb-6 space-y-4">
            ...
            <div className="border rounded-2xl p-4"><p className="font-bold mb-2">Ảnh sản phẩm</p>{newDmxImage && <img src={newDmxImage} className="w-40 rounded-2xl mb-3" />}<input type="file" accept="image/*" onChange={e => { const file = e.target.files?.[0]; if (file) imageToBase64(file, value => setNewDmxImage(value)); }} className="w-full border rounded-2xl px-5 py-4" />{newDmxImage && <button type="button" onClick={() => setNewDmxImage("")} className="mt-3 bg-rose-600 text-white rounded-xl px-4 py-2 font-bold">Xóa ảnh</button>}</div>
            <button onClick={createDmxProduct} className="bg-indigo-600 text-white rounded-2xl px-6 py-4 font-black">Tạo sản phẩm</button>
          </div>
          <div className="space-y-4">{dmxProducts.map(p => <div key={p.id} className="border rounded-3xl p-5 bg-white"><div className="grid md:grid-cols-[140px_1fr] gap-4">{p.image ? <img src={p.image} className="w-32 h-32 object-cover rounded-2xl" /> : <div className="w-32 h-32 rounded-2xl bg-slate-100 grid place-items-center text-slate-400">No image</div>}<div className="space-y-3"><div><h3 className="text-2xl font-black">{p.name}</h3><p className="text-slate-500">Giá bán: {money(p.price)} | Kho còn: {p.stock || 0} | Đã bán: {p.sold || 0}</p><p className={`text-sm font-bold ${p.hidden ? "text-rose-600" : "text-emerald-600"}`}>{p.hidden ? "Đang ẩn" : "Đang hiện"}</p></div><input defaultValue={p.name} onBlur={e => updateDmxProduct(p, { name: e.target.value })} className="w-full border rounded-xl px-3 py-2" placeholder="Tên sản phẩm" /><input defaultValue={p.price} onBlur={e => updateDmxProduct(p, { price: Number(e.target.value) })} className="w-full border rounded-xl px-3 py-2" placeholder="Giá bán" /><textarea defaultValue={p.note || ""} onBlur={e => updateDmxProduct(p, { note: e.target.value })} className="w-full border rounded-xl px-3 py-2" placeholder="Ghi chú / hướng dẫn" /><div className="border rounded-2xl p-3"><p className="font-bold mb-2">Đổi ảnh sản phẩm</p><input type="file" accept="image/*" onChange={e => { const file = e.target.files?.[0]; if (file) imageToBase64(file, value => updateDmxProduct(p, { image: value })); }} className="w-full border rounded-xl px-3 py-2" /></div><div className="flex gap-2 flex-wrap"><button onClick={() => uploadDmxCodes(p)} className="bg-indigo-600 text-white rounded-xl px-4 py-2 font-bold">Upload mã</button><button onClick={() => clearDmxCodes(p)} className="bg-amber-500 text-white rounded-xl px-4 py-2 font-bold">Xóa mã tồn</button><button onClick={() => updateDmxProduct(p, { hidden: !p.hidden })} className={`${p.hidden ? "bg-emerald-600" : "bg-slate-700"} text-white rounded-xl px-4 py-2 font-bold`}>{p.hidden ? "Bỏ ẩn" : "Ẩn"}</button><button onClick={() => deleteDmxProduct(p)} className="bg-rose-600 text-white rounded-xl px-4 py-2 font-bold">Xóa sản phẩm</button></div>{p.codesPreview && p.codesPreview.length > 0 && <div className="bg-slate-100 rounded-xl p-3 text-sm"><b>Mã tồn mẫu:</b><br />{p.codesPreview.join("\n")}</div>}</div></div></div>)}</div>
        </Panel>}
        ...
        {tab === "api" && isAdmin && <Panel title="Cấu hình API nguồn">
          <div className="max-w-4xl space-y-5">
            ...
            <div className="border rounded-3xl p-5 bg-white space-y-4"><div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3"><div><h2 className="text-2xl font-black">chaycodeso3</h2><p className="text-slate-500 text-sm">Nguồn API cũ đang có sẵn trên web.</p></div><label className="flex items-center gap-3 font-bold"><input type="checkbox" checked={providerSettings.chayEnabled} onChange={e => setProviderSettings({ ...providerSettings, chayEnabled: e.target.checked })} />Bật nguồn này</label></div><div className="bg-slate-50 rounded-2xl p-4"><p>Trạng thái key: <b>{providerSettings.hasChayApiKey ? "Đã có" : "Chưa có"}</b></p><p>Key hiện tại: <b>{providerSettings.chayApiKeyMasked || "Chưa cấu hình"}</b></p></div><input type="password" value={chayApiKey} onChange={e => setChayApiKey(e.target.value)} className="w-full border rounded-2xl px-5 py-4" placeholder="Dán API key chaycodeso3 mới vào đây, để trống nếu không đổi" /></div>
            <div className="border rounded-3xl p-5 bg-white space-y-4"><div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3"><div><h2 className="text-2xl font-black">CodeSim</h2><p className="text-slate-500 text-sm">Nguồn API mới thêm vào hệ thống.</p></div><label className="flex items-center gap-3 font-bold"><input type="checkbox" checked={providerSettings.codesimEnabled} onChange={e => setProviderSettings({ ...providerSettings, codesimEnabled: e.target.checked })} />Bật nguồn này</label></div><div className="bg-slate-50 rounded-2xl p-4"><p>Trạng thái key: <b>{providerSettings.hasCodesimApiKey ? "Đã có" : "Chưa có"}</b></p><p>Key hiện tại: <b>{providerSettings.codesimApiKeyMasked || "Chưa cấu hình"}</b></p></div><input type="password" value={codesimApiKey} onChange={e => setCodesimApiKey(e.target.value)} className="w-full border rounded-2xl px-5 py-4" placeholder="Dán API key CodeSim mới vào đây, để trống nếu không đổi" /></div>
