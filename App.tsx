...
const App: React.FC = () => {
  ...
  const initData = async () => {
    ...
    setLoading(true);
    try {
      if (user.role === 'admin' && siteConfig.masterApiKey) {
        try {
      ...
      if (siteConfig.masterApiKey) {
        ...
        if (servicesData.length > 0) {
          const updatedServices = servicesData.map((s: SimService) => ({
            ...s,
            price: Math.round((s.price || 0) * 1.5)
          }));
  ...
  const handleRegister = (username: string, pass: string) => {
    ...
    const newUser: User = { id: 'u-' + Date.now(), username, password: pass, role: 'user', balance: 0 };
  ...
  const handleUpdatePassword = (userId: string, newPass: string) => {
    ...
    if (user?.id === userId) setUser(prev => prev ? { ...prev, password: newPass } : null);
  ...
  const handleSubmitTopup = (amount: number, method: 'BANK' | 'MOMO', content: string) => {
    if (!user) return;
    const newRequest: TopupRequest = { id: 'tr-' + Date.now(), userId: user.id, username: user.username, amount, method, content, status: 'PENDING', createdAt: Date.now() };
  ...
  const handleAddUser = (userData: Partial<User>) => {
    const newUser: User = { id: 'u-' + Date.now(), username: userData.username || '', password: userData.password || '', role: 'user', balance: userData.balance || 0 };
    setAllUsers(prev => [...prev, newUser]);
  ...
  const handleRentSim = async (service: SimService) => {
    ...
    try {
      ...
      if (data && data.otpId) {
        const newOrder: ActiveOrder = { id: data.otpId.toString(), simId: data.simId, phoneNumber: data.phone, serviceName: data.serviceName, otp: null, status: 'WAITING', expiresAt: Date.now() + 15 * 60 * 1000, createdAt: Date.now(), ownerId: user.id };
        ...
        <Login 
          ...
          onLogin={(u, p, remember) => {
            ...
            if (found) {
              setUser(found);
              setActiveTab('dashboard');
              if (remember) localStorage.setItem('otpsim_session_userid', found.id);
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
            ...
            <div className="flex items-center gap-3 flex-1 max-w-md">
               <div className="relative flex-1">
                  <input type="text" placeholder="Tìm tên ứng dụng..." className="w-full pl-5 pr-10 py-3 bg-white border border-slate-200 rounded-2xl outline-none focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 transition-all font-bold text-sm" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} />
               </div>
            </div>
          </div>
          {loading && services.length === 0 ? <div className="py-24 text-center"><div className="w-12 h-12 border-4 border-indigo-600 border-t-transparent rounded-full animate-spin mx-auto mb-6"></div></div> : <ServiceGrid services={filteredServices} onRent={handleRentSim} />}
        </section>
      </div>
      <div className="space-y-8"><AiSupport /></div>
    </div>
  );
  ...
  return (
    <div className="flex min-h-screen bg-[#f8fafc]">
      {toast && <Toast message={toast.message} type={toast.type} />}
      <Sidebar siteName={siteConfig.siteName} logoInitial={siteConfig.logoInitial} balance={balance} role={user.role} activeTab={activeTab} setActiveTab={setActiveTab} onRefresh={initData} />
      <main className="flex-1 p-6 lg:p-10 overflow-y-auto max-w-7xl mx-auto w-full relative">
        <header className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-12">
          <div className="flex flex-col gap-3">
             <div className="flex items-center gap-3">
               <span className="text-[10px] font-black text-indigo-600 bg-indigo-50 px-2.5 py-1 rounded-lg uppercase tracking-widest">{user.role === 'admin' ? 'Quyền Admin' : 'Thành viên'}</span>
               <h1 className="text-2xl md:text-3xl font-black text-slate-800 tracking-tight leading-none capitalize">
                {activeTab === 'users' ? 'Quản lý đại lý' : activeTab === 'settings' ? 'Giao diện hệ thống' : activeTab === 'dashboard' ? 'Bảng điều khiển' : activeTab === 'topup' ? 'Nạp tiền ví' : activeTab === 'topup-manage' ? 'Duyệt đơn nạp' : 'Lịch sử thuê'}
              </h1>
             </div>
            <p className="text-slate-400 font-bold hidden md:flex items-center gap-2">Xin chào, <span className="text-slate-800 font-black">{user.username}</span> <span className="ml-2 px-3 py-1 bg-emerald-50 text-emerald-600 rounded-lg text-xs font-black border border-emerald-100 shadow-sm">{balance.toLocaleString('vi-VN')}đ</span></p>
          </div>
          <button onClick={handleLogout} className="px-6 py-3.5 bg-white text-rose-500 rounded-[1.25rem] border border-slate-200 shadow-sm font-black text-xs uppercase hover:bg-rose-50 transition-colors tracking-widest">Thoát</button>
        </header>

        {activeTab === 'dashboard' && siteConfig.announcement && (
          <div className="mb-10 bg-amber-50 border border-amber-100 p-5 rounded-[2rem] flex items-center gap-4 shadow-sm animate-in slide-in-from-top-4 duration-500">
             <div className="w-10 h-10 bg-amber-400 text-white rounded-full flex items-center justify-center text-xl animate-pulse">📢</div>
             <div className="flex-1"><p className="text-[10px] font-black text-amber-500 uppercase tracking-widest mb-0.5">Thông báo hệ thống</p><p className="text-sm font-bold text-amber-900 leading-tight">{siteConfig.announcement}</p></div>
          </div>
        )}

        <div className="animate-in fade-in duration-500">
          {activeTab === 'users' && user.role === 'admin' ? <UserManagement users={allUsers} orders={orders} onAddUser={handleAddUser} onUpdateBalance={handleUpdateBalance} onUpdatePassword={handleUpdatePassword} onDeleteUser={handleDeleteUser} onDeleteInactiveUsers={handleDeleteInactiveUsers} /> : 
           activeTab === 'topup-manage' && user.role === 'admin' ? <TopupManagement requests={topupRequests} onApprove={handleApproveTopup} onCancel={handleCancelTopup} /> : 
           activeTab === 'settings' && user.role === 'admin' ? <AdminSettings config={siteConfig} onSave={saveConfig} /> : 
           activeTab === 'history' ? <OrderHistory orders={historyOrders} role={user.role} /> : 
           activeTab === 'topup' ? <TopupView config={siteConfig} userRequests={userTopups} onSubmitRequest={handleSubmitTopup} username={user.username} /> : 
